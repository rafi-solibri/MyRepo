...
| Issue | Fix |
| --- | --- |
| POST_FIX_RERUN #261 on merged main: preflight OK; live login still CAPTCHA (GSI timeout + password -> Security Verification exit 6) | Owner-only: refresh LINKEDIN_PASSWORD and GOOGLE_PASSWORD then headed-login + session seed; no new code loop (3/5 same-day re-runs used) |
