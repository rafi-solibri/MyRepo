...
| Issue | Fix |
| --- | --- |
| daily_apply held CDP while spawnSync google_login (deadlock); restore/sync still looked for token not hirist_seeker_enc | Run login helpers as sole CDP client before apply connects; restore/sync Hirist cookies as hirist_seeker_enc+token |
