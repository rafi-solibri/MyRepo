...
| Issue | Fix |
| --- | --- |
| `tools/foundit/daily_apply.js` was login-only scaffold (0 Raven/Falcon applies) | Full runner: Raven public search → `classifyJob` → `userJobInfo`/`applicationStatus` eligibility → Falcon Bearer apply → LinkedIn/ATS handoff; writes `/opt/cursor/artifacts/foundit-apply-report.json` |
