...
If you hit a code-fixable blocker (filters.js / classifyJob, daily_apply.js, Raven/API, ATS handoff), fix durable helpers under tools/foundit or scripts/, append via `bash scripts/append-issue-fix.sh <portal> "issue" "fix"`, commit + push a feature branch, open a ready PR to main and run `bash scripts/auto-merge-fix-pr.sh`. That merge helper then same-day re-runs this Foundit job with the fix (`scripts/rerun-daily-after-fix.sh`) — do not wait for tomorrow's cron. Follow automation-prompts/AUTO_FIX.md. Do not invent applies. Owner-only: login walls, Akamai that needs residential IP, CAPTCHA/OTP.
```
