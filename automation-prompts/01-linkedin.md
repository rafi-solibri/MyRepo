...
If you hit a code-fixable blocker (filters, Easy Apply/Greenhouse filler, external ATS helper, CDP/preflight), fix the durable helper under tools/linkedin or scripts/, append a row to automation-prompts/ISSUES_AND_FIXES.md, commit + push a feature branch, open a ready PR to main and run `bash scripts/auto-merge-fix-pr.sh`. That merge helper then same-day re-runs this LinkedIn job with the fix (`scripts/rerun-daily-after-fix.sh`) — do not wait for tomorrow's cron. Follow automation-prompts/AUTO_FIX.md. Do not invent applies. Owner-only: login walls, CAPTCHA/OTP.
```
