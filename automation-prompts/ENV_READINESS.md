...
| Guest ATS hCaptcha / reCAPTCHA (Hyland iCIMS, Indeed SmartApply) | Set **`CAPSOLVER_API_KEY`** (preferred) or `TWOCAPTCHA_API_KEY` on the same Secrets tab (or `.cursor/job-apply-secrets.env`). Career-portal Apply then injects the token and continues the form. DataDome (Experian / Blackbaud / Palo Alto) is not a token widget — home/residential session only. |
