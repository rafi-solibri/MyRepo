...
| LinkedIn unattended login | Set Environment Secrets `LINKEDIN_EMAIL` + `LINKEDIN_PASSWORD` on https://cursor.com/dashboard/cloud-agents/environments/e/545c2557-9097-11f1-ba66-0e7d0216e441 (Secrets tab). Cloud AWS IPs often still hit CAPTCHA after password — then once: headed login on home PC (`bash scripts/home-headed-login.sh linkedin`) → `bash scripts/refresh-portal-session-seed.sh linkedin` → **Save snapshot**. Optional local file (gitignored): `.cursor/job-apply-secrets.env` sourced by `scripts/load-job-secrets.sh`. |
| Guest ATS hCaptcha / reCAPTCHA (Hyland iCIMS, Indeed SmartApply) | **Free (preferred if you will not pay a solver):** on your home PC run `bash scripts/home-headed-careers-apply.sh` — the helper fills the form and waits for you to click hCaptcha in the visible Chrome window (`ATS_CAPTCHA_WAIT_SEC`, default 300s). Cloud headless cannot click it. **Optional paid:** `CAPSOLVER_API_KEY` or `TWOCAPTCHA_API_KEY` on the Secrets tab. DataDome (Experian / Blackbaud / Palo Alto) is not a token widget — home/residential session only. |
