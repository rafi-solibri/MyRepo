...
If you hit a code-fixable blocker (filters.js / skipReason, daily_apply.js, resume path), fix durable helpers under tools/instahyre or scripts/, append automation-prompts/ISSUES_AND_FIXES.md, commit + push a feature branch, open a draft PR to main. Follow automation-prompts/AUTO_FIX.md. Do not invent applies. Owner-only: login walls, CAPTCHA/OTP, HTTP 429 that needs backoff already in runner.
```
