 ...
 */
const SKIP_TITLE_RE =
  /\b(qa engineer|quality assurance|quality engineer|quality engineering|quality architect|quality solution architect|\bqe architect\b|sdet|tosca|test automation architect|test engineer|test lead|\bqa lead\b|embedded\b|firmware|intern(?!et)|fresher|salesforce|agentforce|servicenow|coupa|pega|lead system architect|\blsa\b|appian|anaplan|celonis|power platform|guidewire|sap\b|dynamics|\bd365\b|workday hms|revit|\bbarch\b|hubspot|\bsre\b|site reliability|\bdevops\b|platform sre|network operations|network ops|network support|network cisco|network\s+(?:solution\s+)?architect|cisco\s+meraki|\bmeraki\b|sd-?\s*wan|l2\s*\/\s*l3|data\s*center|civil\b|structural|substation|attack surface|cyber\s*security|cybersecurity|cyber architecture|infosec|penetration|red team|soc analyst|security operations|threat hunter|\bmdr\b|\bedr\b|security\s+engineer|security\s+architecture|observability|\bdatadog\b|infrastructure engineer|analog\s*ic|\bvlsi\b|digital verification|\basic\b|\bfpga\b|\bdft\b|\batpg\b|\bjtag\b|\bmbist\b|\btessent\b|\btestmax\b|mulesoft|mule\s*soft|ms\s*fabric|microsoft\s*fabric|\bsynapse\b|\bdatabricks\b|datalake|data\s*lake|\bnetcool\b|big\s*data|bigdata|oracle\s+epm|\bepm\b|\bpbcs\b|\bepbcs\b|sharepoint|\btableau\b|\bcopilot\s+architect\b|ms\s+copilot|\bui\s+architect\b|teamcenter|windchill|\bplm\b|\bimds\b|it\s+shared\s+services|aws\s+infra(?:structure)?(?:\s+architect)?|\bios\b|\baccounting\b|commercial analytics|\biiot\b|industrial\s+i+ot|\biot\s+(?:engineer|architect)|telecom\s+billing|avp\s+of\s+sales|(?:vp|avp|director|head)\s+(?:of\s+)?sales|sales\s+(?:director|manager|lead|executive|avp|vp))\b/i;
