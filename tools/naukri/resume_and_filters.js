 ...
 */
const SKIP_TITLE_RE =
  /\b(qa engineer|quality assurance|quality engineer|quality engineering|quality architect|quality solution architect|\bqe architect\b|sdet|tosca|test automation architect|testing\s+services|embedded\b|firmware|intern(?!et)|fresher|salesforce|agentforce|service\s*now|\bitom\b|\bcmdb\b|\bitam\b|success\s*factors?|\bkdb\+?\b|coupa|pega|lead system architect|\blsa\b|appian|anaplan|celonis|power platform|guidewire|sap\b|dynamics|\bd365\b|workday hms|revit|\bbarch\b|hubspot|\bsre\b|site reliability|\bdevops\b|platform\s+engineering|platform sre|network operations|network ops|network support|network cisco|network\s+(?:solution\s+)?architect|cisco\s+meraki|\bmeraki\b|sd-?\s*wan|l2\s*\/\s*l3|data\s*center|civil\b|structural|substation|\bpiping\b|\boffshore\b.{0,48}\b(electrical|instrumentation|control|piping)\b|\b(electrical|instrumentation|control|piping)\b.{0,48}\boffshore\b|control\s*&?\s*instrumentation|\binstrumentation\b|attack surface|cyber\s*security|cybersecurity|cyber architecture|\bcyberark\b|infosec|penetration|red team|soc analyst|security operations|threat hunter|\bmdr\b|\bedr\b|security\s+engineer|security\s+architecture|observability|\bdatadog\b|infrastructure engineer|analog\s*ic|\bvlsi\b|digital verification|\basic\b|\bfpga\b|\bdft\b|\batpg\b|\bjtag\b|\bmbist\b|\btessent\b|\btestmax\b|mulesoft|mule\s*soft|\bapigee\b|ms\s*fabric|microsoft\s*fabric|(?<!service\s)fabric\s+architect|\bsynapse\b|\bdatabricks\b|\bsnowflake\b|\bdbt\b|datalake|data\s*lake|\bnetcool\b|big\s*data|bigdata|oracle\s+epm|\bepm\b|\bpbcs\b|\bepbcs\b|\bxstore\b|sharepoint|\btableau\b|\baem(?:aacs)?\b|adobe\s+experience\s+manager|\bcopilot\s+architect\b|ms\s+copilot|\bui\s+architect\b|product\s+designer|ux\s+designer|ui\s+designer|graphic\s+designer|(?:technical\s+)?project\s+manager|teamcenter|windchill|\bplm\b|\bimds\b|it\s+shared\s+services|aws\s+infra(?:structure)?(?:\s+architect)?|aws\s+platform(?:\s+architect)?|\bios\b|\baccounting\b|financial\s+planning|\bfp\s*&?\s*a\b|\biiot\b|industrial\s+i+ot|\biot\s+(?:engineer|architect)|telecom\s+billing|avp\s+of\s+sales|(?:vp|avp|director|head)\s+(?:of\s+)?sales|sales\s+(?:director|manager|lead|executive|avp|vp))\b/i;
...
const PURE_AI_DATA_RE =
  /\b(ai\s+architect|artificial\s+intelligence\s+architect|ai\s+agent|ai\s+engineer(?:ing)?(?:\s+manager|\s+lead)?|ai\s+engineering\s+manager|ai\s+tech(?:nical)?\s+lead|engineering\s+manager\s*-?\s*ai|ai\s+solution\s+architect|full\s*stack\s+ai\s+manager|ai\s+manager|architect[^.\n]{0,48}\b(ai|ml)\b|data\s*(?:&|and)\s*ai|ml\s+engineer|gen\s*-?\s*ai|genai|agentic\s+ai|\bgemini\b|\bllm\b|enterprise\s+platform\s+architect[^.\n]{0,24}\b(ai|ml|gemini|llm)\b|data\s+scientist|data\s+engineer|data\s+engineering|data\s+architect|data\s+solution\s+architect|data\s+architecture|gcp\s+infra(?:structure)?(?:\s+architect)?|aws\s+infra(?:structure)?(?:\s+architect)?|cloud\s+infra(?:structure)?(?:\s+architect)?)\b/i;
...
const NON_DOTNET_PRIMARY_RE =
  /\b(java|j2ee|spring boot|golang|go lang|python|node\.?js|\bvue\b|mean\b|mern\b|ruby on rails|php|oracle apps|oracle fusion|oracle\s+dba|oracle cloud|oracle\s+epm|oracle\s+paas|oracle\s+hcm|oracle\s+erp|oracle\s+ebs|oracle\s+apex|\bapex\s+(?:solution\s+)?architect|\bnetsuite\b|fusion\s+hcm|abap|mainframe|cobol|\bas400\b|ibm\s*i)\b/i;
