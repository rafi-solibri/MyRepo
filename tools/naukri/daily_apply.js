...
function searchUrl(q, age) {
  ...
  const l = encodeURIComponent("hyderabad/secunderabad, remote");
  return `https://www.naukri.com/${slug}-jobs-in-hyderabad-secunderabad?k=${k}&l=${l}&experience=12&jobAge=${age}`;
}
  ...
  try {
    for (const age of JOB_AGES) {
      ...
      for (const q of QUERIES) {
        if (report.applied.length >= MAX_APPLIES) break;
        const url = searchUrl(q, age);
        report.queriesRun.push({ q, age, url });
        await page
    ...
    // Recommended inventory pass when search burst is thin
    if (report.applied.length < 15) {
      await page
        .goto("https://www.naukri.com/mnjuser/recommendedjobs", {
