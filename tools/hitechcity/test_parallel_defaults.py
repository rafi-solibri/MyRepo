...
def test_daily_apply_setdefault_parallel_tabs():
    ...
    assert 'os.environ.setdefault("ATS_OWNER_FOCUS_EVERY_SEC", "0")' in src
...
def test_home_headed_exports_parallel():
    ...
    assert 'ATS_OWNER_FOCUS_EVERY_SEC="${ATS_OWNER_FOCUS_EVERY_SEC:-0}"' in text


def test_workers_keep_ten_tab_budget():
    src = Path(__file__).with_name("careers_parallel.py").read_text(encoding="utf-8")
    worker_src = src.split("def run_parallel")[0]
    assert 'PARALLEL_TABS"] = "1"' not in worker_src
    assert "PARALLEL_WORKER" in worker_src
    complete = Path(__file__).resolve().parents[2] / "tools" / "ats" / "complete.py"
    assert "focus_every > 0" in complete.read_text(encoding="utf-8")
...
if __name__ == "__main__":
    ...
    test_workers_keep_ten_tab_budget()
    print("ok")
