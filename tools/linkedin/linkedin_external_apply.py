...
def restriction_skip_payload() -> dict | None:
    ...
    try:
        from tools.linkedin.restriction import should_skip_linkedin_for_restriction  # pragma: allowlist secret
    except Exception:
        try:
            from restriction import should_skip_linkedin_for_restriction  # type: ignore  # pragma: allowlist secret
        except Exception:
            return None
    return should_skip_linkedin_for_restriction()  # pragma: allowlist secret
...
def main() -> None:
    ...
    if skip_restr:
        res = ExtResult(
            status="blocked",
            reason=f"linkedin_temporarily_restricted until {skip_restr.get('lift_utc')}",  # pragma: allowlist secret
        )
        out = {
        ...
        print(json.dumps({"error": "linkedin_temporarily_restricted", **skip_restr}), flush=True)  # pragma: allowlist secret
        raise SystemExit(7)
