...
def _heal_google_auth_pages(ctx, out: dict | None = None) -> str | None:
    ...
    if not pages:
        return None
    for popup in pages:
        # Password form FIRST. /challenge/pwd is "Enter your password", not 2FA —
        # treating it as ASK_OWNER_GOOGLE_2FA burns the method timeout (300s)
        # and never fills GOOGLE_PASSWORD.
        if _page_needs_google_password(popup):
            ...
            if ident_result == "ok":
                return "ok"
            continue
        # Real 2FA / authenticator / phone prompt (not password).
        try:
            from tools.google_2fa_prompt import (
                is_google_2fa_challenge,
                wait_owner_google_2fa,
