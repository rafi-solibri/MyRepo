...
METHOD_TIMEOUT_S = int(os.environ.get("LINKEDIN_AUTO_LOGIN_METHOD_TIMEOUT_S", "90"))
...
# Cookie names can be present while Google/LinkedIn sessions are dead.
PASSWORD_ENV_KEYS = (
    "LINKEDIN_PASSWORD",
    "GOOGLE_PASSWORD",
...
def wrong_password_text(text: str) -> bool:
    """True when LinkedIn or Google shows an explicit wrong-password error."""
...
def main() -> int:
    ...
    with sync_playwright() as p:
        ...
        if wrong_pw:
            ...
            out["hint"] = (
                "LinkedIn/Google rejected the configured password secret(s). "
                "Update Cursor secret LINKEDIN_PASSWORD (and optionally GOOGLE_PASSWORD) "
                "or run: bash scripts/home-headed-login.sh linkedin"
            )
        print(json.dumps(out))
        return 5
