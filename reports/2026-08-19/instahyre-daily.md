# Instahyre daily — 2026-08-19 (cloud)

Mohammed Abdul Rafi Ahmed | Expected **65 LPA** / Current **52 LPA** | Hyd + Remote | resume `Rafi_Resume.docx`

Logged in via CDP (`sessionid` live on `/candidate/opportunities/`). No invented applies — Instahyre `apply` API + interested counts.

## Totals

| Metric | First pass | After filter fix |
| --- | ---: | ---: |
| Applied (Instahyre in-app) | 1 | +1 |
...
- **DAT Freight & Analytics** — SDE - 3 (Work From Home) — path: Instahyre; UI: application_sent — job 423301 / opp 6184011665
- **Amazon** — Sr. Software Engineer (Hyderabad) — path: Instahyre; UI: application_sent — job 439255 / opp 6184030768 *(second pass after filter fix)*
...
- Tests: `node tools/instahyre/filters.location.test.js`
- Artifact (first): `/opt/cursor/artifacts/instahyre-apply-report.json`
- Artifact (re-run): `/opt/cursor/artifacts/instahyre-apply-report-rerun.json`
